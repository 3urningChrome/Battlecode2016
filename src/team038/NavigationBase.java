...
public class NavigationBase extends AusefulClass{
	static MapLocation destination = current_location;
	...
	static NavType bugNav = NavType.DUCKBUG;
	...
	public static MapLocation navigate_to_destination(MapLocation A){
		return bugNav.navigate_to_destination(A);
	}
	...
	public static void log_new_collision(){
		...
		//seems we have issues here. probably not pathable. so start again.
		if(i >= MEMORY_SIZE){
			initialise_ready_for(destination);
			i+=1;
		}
...
// if wall no longer there, then quit bugging?		
		...
		if(rc.canMove(direction_of_wall)){
			initialise_ready_for(destination);
