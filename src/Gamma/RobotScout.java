...
public class RobotScout extends AusefulClass {
	...
	//Navigation, Find next Destination.
		...
		if(turns_in_current_direction >= max_scouting_direction_turns){
			...
			scouting_direction = (int) ((rnd.nextDouble()*8)%8);
		}
		...
		if(Scanner.can_see_targets()){
			...
			if(closest_hostile.location.distanceSquaredTo(current_location) <= Math.max(closest_hostile.type.sensorRadiusSquared,closest_hostile.type.attackRadiusSquared) && closest_hostile.type != RobotType.ZOMBIEDEN)
		...
		if(Scanner.can_see_turrets())
			Communications.log_turret_exclusion();
