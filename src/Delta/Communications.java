...
//					chatter_zones.add(message_location);
			...
			case Part_Location_Data:
				if(my_type==RobotType.ARCHON){
					...
					System.out.println("Part Location: " + data_location);
	...
	private static void broadcast_basic() throws GameActionException {
		...
		rc.broadcastSignal(signal_strength);
	...
	private static int get_signal_strength(){
		...
		double my_core_delay = rc.getCoreDelay() - (int) rc.getCoreDelay();
		double my_weapon_delay = rc.getWeaponDelay() - (int) rc.getWeaponDelay();
